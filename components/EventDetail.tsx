...
export default function EventDetail({ event, onBuyTicket, onSeeHistory, onBack }: EventDetailProps) {
  let formattedDate = ''
  let formattedTime = ''
  ...
  return (
    <motion.div
      ...
      style={{
        backgroundImage: event.image_url ? `url('${event.image_url}')` : "url('/anchor-landing-bg.jpg')",
      }}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
